...
fn sell_failed_not_owner() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(KittiesModule::create(Origin::signed(1)));
		assert_noop!(
			KittiesModule::sell(Origin::signed(2), 0, Some(1000)),
			Error::<Test>::NotOwner
		);
	});
}
...
fn buy_failed_token_not_enough() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			KittiesModule::buy(Origin::signed(account_id_2), kitty_id),
			Error::<Test>::TokenNotEnough
		);
	});
}
...
fn buy_failed_invalid_kitty_id() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			KittiesModule::buy(Origin::signed(account_id_2), 2),
...
fn buy_failed_buy_self() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			KittiesModule::buy(Origin::signed(account_id_1), kitty_id),
			Error::<Test>::NoBuySelf
		);
	});
}
